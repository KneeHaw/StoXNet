...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        analysis = False
        # chosen_list = [1, 1, 2, 4, 8, 16] * 3
        # chosen_list = [16, 8, 4, 2, 1, 1] * 3
        chosen_list = [1, 1, 1, 1, 1, 1] * 3
        print(layer_importance)

        def get_mapped():
            if max(layer_importance) > 1.2 and analysis is True:
                # mapped_steps = args.max_time_steps * torch.tanh((torch.tensor(layer_importance) - 1) / (max(layer_importance) - 1))

                # mapped_a = args.max_ab * torch.tanh((torch.tensor(layer_importance) - 1) / (max(layer_importance) - 1))
                # mapped_w = args.max_wb * torch.tanh((torch.tensor(layer_importance) - 1) / (max(layer_importance) - 1))
                mapped_a = [args.max_ab] * 18
                mapped_w = [args.max_wb] * 18
            elif max(layer_importance) < 1.2 and analysis is True:
                mapped_steps = [args.max_time_steps] * 18
                mapped_a = [args.max_ab] * 18
                mapped_w = [args.max_wb] * 18
            else:
                # mapped_steps = [args.max_time_steps] * 18
                mapped_steps = chosen_list
                mapped_a = [args.max_ab] * 18
                mapped_w = [args.max_wb] * 18
            return mapped_steps, mapped_a, mapped_w
        time_steps, mapped_ab, mapped_wb = get_mapped()
        ...
        for i in range(3):
            ...
            model.module.layer1[i].conv1.a_bits = max(mapped_ab[0 + (i * 6)], 1)
            model.module.layer1[i].conv2.a_bits = max(mapped_ab[1 + (i * 6)], 1)
            model.module.layer2[i].conv1.a_bits = max(mapped_ab[2 + (i * 6)], 1)
            model.module.layer2[i].conv2.a_bits = max(mapped_ab[3 + (i * 6)], 1)
            model.module.layer3[i].conv1.a_bits = max(mapped_ab[4 + (i * 6)], 1)
            model.module.layer3[i].conv2.a_bits = max(mapped_ab[5 + (i * 6)], 1)

            model.module.layer1[i].conv1.w_bits = max(mapped_wb[0 + (i * 6)], 1)
            model.module.layer1[i].conv2.w_bits = max(mapped_wb[1 + (i * 6)], 1)
            model.module.layer2[i].conv1.w_bits = max(mapped_wb[2 + (i * 6)], 1)
            model.module.layer2[i].conv2.w_bits = max(mapped_wb[3 + (i * 6)], 1)
            model.module.layer3[i].conv1.w_bits = max(mapped_wb[4 + (i * 6)], 1)
            model.module.layer3[i].conv2.w_bits = max(mapped_wb[5 + (i * 6)], 1)
