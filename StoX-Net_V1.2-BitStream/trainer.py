...
parser.add_argument('-b', '--batch-size', default=40, type=int,  # use different batch sizes
...
best_prec1 = 0
Log_Vals = open("save_temp/StoX400ep-ReLUNet-09Lr-256arr-1024batch-LeakyReLU.txt", 'w')
...
def main(rank=None, world_size=None):
    ...
    # model = DDP(resnet.__dict__[args.arch]().to(rank), device_ids=[rank])
    ...
    train_loader = torch.utils.data.DataLoader(dataset=datasets.CIFAR10(root='./data',
                                                    train=True, transform=transforms.Compose([
                                                       transforms.RandomHorizontalFlip(),
                                                       transforms.RandomCrop(32, 4),
                                                       transforms.ToTensor(), normalize]), download=True), batch_size=args.batch_size, shuffle=True,
                                               num_workers=args.workers, pin_memory=True, drop_last=True)
    # sampler=DistributedSampler(datasets.CIFAR10(root='./data',
    # train=True, transform=transforms.Compose([
    #    transforms.RandomHorizontalFlip(),
    #    transforms.RandomCrop(32, 4),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            ...
            }, is_best, filename=os.path.join(args.save_dir, 'StoX400ep-ReLUNet-09Lr-256arr-1024batch-LeakyReLU.th'))
